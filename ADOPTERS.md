...
| [HeroDevs](https://www.herodevs.com) | HeroDevs uses the Paketo Java buildpacks (`spring-boot` in particular) to facilitate builds of our internal tooling. Using Paketo Buildpacks was a logical step when we started publishing OCI images to registries: it's the default way to build OCI images for Spring Boot |
| [IBM](https://cloud.ibm.com/docs/codeengine?topic=codeengine-plan-build#build-buildpack-strat) | IBM uses Paketo-based buildpacks for its serverless containers offering [IBM Cloud Code Engine](https://www.ibm.com/products/code-engine). Users can chose between Dockerfile and Buildpacks to containerize their source code to run applications reachable over HTTPS, batch jobs, or large-scale compute-intensive workload. |
